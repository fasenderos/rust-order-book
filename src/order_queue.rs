...
use crate::{
    math::math::{safe_add, safe_add_sub, safe_sub},
    order::OrderId,
};
...
impl OrderQueue {
    ...
    pub fn update(&mut self, id: OrderId, old_quantity: u64, new_quantity: u64) {
        self.volume = safe_add_sub(self.volume, new_quantity, old_quantity);
    }

    /// removes order from the queue
    pub fn remove(&mut self, id: OrderId, quantity: u64) {
...
//         let items: Vec<OrderId> = q.orders.iter().map(|x| *x).collect();
